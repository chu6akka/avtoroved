...
from PyQt6.QtWidgets import (
    ...
    QHeaderView, QLabel, QLineEdit, QPushButton, QFileDialog, QMessageBox
)
...
class MorphologyTab(QWidget):
    ...
    def populate(self, tokens: List[TokenInfo], text: str, morph_indices: dict = None):
        ...
        for row_idx, tok in enumerate(words):
            # Используем char_start/char_end если доступны, иначе ищем по тексту
            if hasattr(tok, 'char_start') and (tok.char_start > 0 or tok.char_end > 0):
                span = (tok.char_start, tok.char_end)
                cursor = tok.char_end
            else:
