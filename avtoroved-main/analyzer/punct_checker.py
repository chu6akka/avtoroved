...
# Версия набора правил — пишется в audit_log протокола для воспроизводимости.
# Менять при любом изменении правил ниже.
RULES_VERSION = "1.1"
...
_COMPOUND_CONJUNCTIONS: List[tuple] = [
    ...
    (re.compile(r"\bпотому\s+как\b",           re.IGNORECASE), "потому как"),
    (re.compile(r"\bтак\s+как\b",              re.IGNORECASE), "так как"),
    (re.compile(r"\bоттого\s+что\b",           re.IGNORECASE), "оттого что"),
    (re.compile(r"\bиз-за\s+того\s+что\b",     re.IGNORECASE), "из-за того что"),
    (re.compile(r"\bблагодаря\s+тому\s+что\b", re.IGNORECASE), "благодаря тому что"),
    (re.compile(r"\bввиду\s+того\s+что\b",     re.IGNORECASE), "ввиду того что"),
    (re.compile(r"\bвследствие\s+того\s+что\b", re.IGNORECASE), "вследствие того что"),
    (re.compile(r"\bв\s+связи\s+с\s+тем\s+что\b", re.IGNORECASE), "в связи с тем что"),
    ...
    (re.compile(r"\bперед\s+тем\s+как\b",      re.IGNORECASE), "перед тем как"),
    (re.compile(r"\bдо\s+того\s+как\b",        re.IGNORECASE), "до того как"),
    (re.compile(r"\bпосле\s+того\s+как\b",     re.IGNORECASE), "после того как"),
    (re.compile(r"\bпо\s+мере\s+того\s+как\b", re.IGNORECASE), "по мере того как"),
    ...
    # «то есть» / «так что»: лукэраунды отсекают «кто-то есть», «так что-то».
    (re.compile(r"(?<![-\w])то\s+есть\b(?![-\w])", re.IGNORECASE), "то есть"),
    (re.compile(r"\bтак\s+что\b(?![-\w])",     re.IGNORECASE), "так что"),
]

# Вводные слова / конструкции (в начале предложения без запятой после).
# Намеренно НЕ включены омонимичные в начале предложения: «однако» (союз —
# запятая после не нужна), «наконец» (обстоятельство), «действительно»,
# «значит», «допустим» (глагольные/вопросительные обороты) — «не навреди».
_INTRODUCTORY_WORDS: List[str] = [
    ...
    "кажется", "безусловно", "бесспорно", "несомненно",
    "во-первых", "во-вторых", "в-третьих", "в-четвёртых", "в-четвертых",
    "итак", "следовательно", "таким образом", "стало быть",
    "например", "к примеру", "в частности", "в особенности",
    "кстати", "к слову", "между прочим", "впрочем",
    "с одной стороны", "с другой стороны",
    "в общем", "в целом", "в принципе", "по сути", "по существу",
    "к сожалению", "к счастью", "как известно", "как правило",
    "иными словами", "другими словами", "короче говоря", "честно говоря",
    "откровенно говоря", "собственно говоря", "мягко говоря",
    "по правде говоря", "судя по всему",
]
