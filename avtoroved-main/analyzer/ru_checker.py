...
_INF_MARKER = (r"надо|нужно|нельзя|можно|невозможно|должн[аоы]?|может|могут|мог|"
               r"буд[еу]т|стоит|стоило|следует|следовало|хоч(?:ет|ут)|хотел[аио]?|"
               r"начал[аио]?|начн(?:ёт|ут)|перестал[аио]?|прекратил[аио]?|"
               r"бросил[аио]?|продолжил[аио]?|закончил[аио]?|"
               r"собира(?:ется|ются)|пыта(?:ется|ются)|стара(?:ется|ются)|"
               r"стрем(?:ится|ятся)|готов[аоы]?|стал[аио]?|продолжа(?:ет|ют)|"
               r"люб(?:ит|ят)|боится|боятся|придётся|придется|пришлось|"
               r"приходится|удалось|удаётся|удается|полагается|положено|"
               r"запрещено|разрешено|принято|пора|лень|привык(?:ла|ли)?|"
               r"устал[аио]?|решил[аио]?|обещал[аио]?|забыл[аио]?|успел[аио]?|"
               r"намерен[аы]?|способ(?:ен|на|ны)|мечта(?:ет|ют)|планиру(?:ет|ют)|"
               r"советует|советуют|рекомендуется|позволя(?:ет|ют)|"
               r"помога(?:ет|ют)|меша(?:ет|ют)")
_RE_TSYA_AFTER_INF = re.compile(
    r"\b(" + _INF_MARKER + r")\s+([а-яё]+[тч]ся)\b", re.IGNORECASE)
# Личное местоимение → личная форма («-тся»). Правая защита от инверсии:
# «Он учиться не хочет» / «Он учиться будет» — инфинитив корректен, если
# дальше идёт отрицание или маркер инфинитива.
_RE_TSYA_AFTER_PRON = re.compile(
    r"\b(он|она|оно)\s+([а-яё]+ться)\b"
    r"(?!\s+не\b)(?!\s+(?:" + _INF_MARKER + r")\b)", re.IGNORECASE)
...
_TAUT_STOP_LEMMAS = {
    ...
    "хотеть", "знать", "думать", "видеть", "идти", "делать", "сделать",
    "давать", "дать", "иметь", "место", "случай", "конец", "начало",
    "сторона", "образ", "вид", "новый", "большой", "хороший",
    "должный", "нужный", "день", "слово", "вопрос",
}
...
_PLEONASMS = (
    ...
    (r"взаимн\w{1,3}\s+сотрудничеств\w{1,3}", "взаимное сотрудничество"),
    ...
    (r"неожиданн\w{1,3}\s+сюрприз\w{0,3}", "неожиданный сюрприз"),
    (r"бесплатн\w{1,3}\s+подар(?:ок|ка|ку|ком|ке|ки|ков)", "бесплатный подарок"),
    (r"молод\w{1,3}\s+юнош\w{1,3}", "молодой юноша"),
    (r"пожил\w{1,3}\s+старик\w{0,3}", "пожилой старик"),
    (r"прейскурант\w{0,3}\s+цен\b", "прейскурант цен"),
    (r"сервисн\w{1,3}\s+обслуживани\w{1,3}", "сервисное обслуживание"),
    (r"главн\w{1,3}\s+приоритет\w{0,3}", "главный приоритет"),
    (r"отступить?\s+назад|отступил\w{0,2}\s+назад", "отступить назад"),
    (r"подн[яи]\w{1,5}\s+вверх", "подняться вверх"),
    (r"спус[тк]\w{1,6}\s+вниз", "спуститься вниз"),
    (r"(?:сам\w{1,2}|наиболее)\s+оптимальн\w{1,3}", "самый/наиболее оптимальный"),
    (r"полн\w{1,3}\s+аншлаг\w{0,3}", "полный аншлаг"),
    (r"кивнул\w{0,2}\s+головой", "кивнул головой"),
    (r"жестикулировал\w{0,2}\s+руками", "жестикулировал руками"),
    (r"\b(?:январ|феврал|март|апрел|ма[йяе]|июн|июл|август|сентябр|"
     r"октябр|ноябр|декабр)\w{0,4}\s+месяц\w{0,2}\b", "«… месяц» после названия месяца"),
)
